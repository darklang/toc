...
func printStats(w *bufio.Writer, cfg *Config, stats *Stats) {
	w.WriteString("\n\n")
	w.WriteString("*Generated by [toc](https://github.com/darklang/toc)*")
	w.WriteString(" - *")
	w.WriteString(strconv.Itoa(stats.emptyDirCount))
	w.WriteString(" dirs and ")
	w.WriteString(strconv.Itoa(stats.emptyFileCount))
	w.WriteString(" files still need descriptions*\n\n")
}

func printLayout(w *bufio.Writer, cfg *Config, stats *Stats, indent int, layout Layout) {
	// Keep some stats
	if layout.isDir {
		stats.dirCount += 1
	} else {
	...
	if layout.description == "" {
		if layout.isDir {
			stats.emptyDirCount += 1
		} else {
			stats.emptyFileCount += 1
		}
	}
	if layout.filename != "" {
